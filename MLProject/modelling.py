...
def train_model(model, model_name, X_train, X_test, y_train, y_test, feature_columns):
    ...
    plt.tight_layout()
    plot_path = f'{model_name}_evaluation.png'
...
@click.command()
@click.option('--data_path', default='weatherdata_preprocessing.csv', help='Path to the dataset')
@click.option('--test_size', default=0.2, help='Test size for train-test split')
@click.option('--random_state', default=42, help='Random state for reproducibility')
@click.option('--model', default='all', help='Model to train: all, random_forest, linear_regression, decision_tree')
@click.option('--n_estimators', default=100, help='Number of estimators for Random Forest')
@click.option('--max_depth', default=10, help='Max depth for tree-based models')
def main(data_path, test_size, random_state, model, n_estimators, max_depth):
    """
    Main function untuk menjalankan eksperimen MLflow
    """
    ...
    if model == 'all' or model == 'random_forest':
    ...
    if model == 'all' or model == 'linear_regression':
        models["LinearRegression"] = LinearRegression()
    
    if model == 'all' or model == 'decision_tree':
    ...
    for model_name, model_instance in models.items():
        with mlflow.start_run(run_name=f"{model_name}_experiment"):
            ...
            mlflow.sklearn.autolog(disable=True)  # Disable to avoid conflicts
            
            trained_model = train_model(
                model_instance, model_name, X_train, X_test, y_train, y_test, feature_columns
            )
